...
describe('InternalLinkSuggestions', () => {
  beforeEach(() => {
    ...
    mockUseEntityPropBinding.mockReturnValue(['wordpress seo', jest.fn()]);
  ...
  it('should display suggestions with title, URL, and relevance score', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('https://example.com/seo-best-practices')).toBeInTheDocument();
      expect(screen.getByText('95%')).toBeInTheDocument();
      
      expect(screen.getByText('WordPress SEO Guide')).toBeInTheDocument();
      expect(screen.getByText('https://example.com/wordpress-seo-guide')).toBeInTheDocument();
  ...
  it('should handle API errors gracefully', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Unable to load link suggestions. Please try again later.')).toBeInTheDocument();
  ...
  it('should display empty state when no suggestions found', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('No internal link suggestions found for this keyword.')).toBeInTheDocument();
    });
  });
